...
If Len(txtAnswer) = 20 Then
	'update tblStressSchedule with the answers
	txtAnswers = cStr(Request.Form("fAnswers")) + cStr(txtAnswer)
	'Response.Write(txtAnswers & "<br>")
	
	sqlUpdate = "UPDATE SCUser.tblStressSchedule SET Answers = '" & cStr(txtAnswers) & "', TestDate = '" & cStr(Date) & "' " &_
		"WHERE StressScheduleID = " & Request.Form("fTID")
